...
class GoogleSheetsService {
  ...
  Future<http.Response> _postWithRedirect(String url, Map<String, String> headers, String body) async {
    final client = http.Client();
    try {
      final request = http.Request('POST', Uri.parse(url))
        ..headers.addAll(headers)
        ..body = body
        ..followRedirects = false; // Tắt tự động theo dõi chuyển hướng của client
  ...
  Future<bool> backupToGoogleSheets(String webAppUrl) async {
    if (webAppUrl.trim().isEmpty) {
      developer.log('Google Sheets Sync: URL rỗng', name: 'GoogleSheetsService');
      return false;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (resData['status'] == 'success') {
          developer.log('✅ Google Sheets: Sao lưu thành công!', name: 'GoogleSheetsService');
          return true;
        } else {
          developer.log('❌ Google Sheets: Script báo lỗi: ${resData['message']}', name: 'GoogleSheetsService');
        }
      } else {
        developer.log('❌ Google Sheets: HTTP status ${response.statusCode}', name: 'GoogleSheetsService');
      }
      return false;
    } catch (e) {
      developer.log('❌ Google Sheets Sync Error: Lỗi sao lưu: $e', name: 'GoogleSheetsService');
      return false;
    }
  }
  ...
  Future<bool> restoreFromGoogleSheets(String webAppUrl) async {
    ...
    try {
      final uri = Uri.parse(webAppUrl).replace(queryParameters: {'action': 'restore'});
      ...
      if (response.statusCode == 200) {
        ...
        if (resData['status'] == 'success' && resData['data'] != null) {
          ...
          developer.log('✅ Google Sheets: Khôi phục dữ liệu thành công!', name: 'GoogleSheetsService');
          return true;
        } else {
          developer.log('❌ Google Sheets: Script báo lỗi hoặc không có dữ liệu: ${resData['message']}', name: 'GoogleSheetsService');
        }
      } else {
        developer.log('❌ Google Sheets: HTTP status ${response.statusCode}', name: 'GoogleSheetsService');
      }
      ...
      developer.log('❌ Google Sheets Sync Error: Lỗi khôi phục: $e', name: 'GoogleSheetsService');
      return false;
    }
  }
}
